...
/**
 * Log an error, show a blocking error dialog, then exit without deleting user data.
 */
pub fn handle_fatal_error<R: Runtime>(app_handle: &tauri::AppHandle<R>, err: MuseeksError) {
    error!("Something went terribly wrong: {:?}", err);
    app_handle
        .dialog()
        .message(format!("Something went terribly wrong: \"{}\". Your YifuMusic data was not changed. If this keeps happening, please report an issue.", err))
